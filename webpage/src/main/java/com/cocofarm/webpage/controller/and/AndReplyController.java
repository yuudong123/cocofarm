...
@RestController
@RequestMapping("reply/*.and")
public class AndReplyController {

    @Autowired
    ReplyService service;

    @PostMapping(value = "selectreplylist", produces = "text/html;charset=utf-8")
    public String selectReplyListAnd(int board_no) {
        ArrayList<ReplyVO> list = service.selectList(board_no);
        return new Gson().toJson(list);
    }

    @PostMapping(value = "selectanswer", produces = "text/html;charset=utf-8")
    public String selectAnswerAnd(int board_no) {
        ReplyVO vo = service.selectAnswer(board_no);
        return new Gson().toJson(vo);
    }

    @PostMapping(value = "insertreply", produces = "text/html;charset=utf-8")
    public void insertReplyAnd(ReplyVO vo) {
        service.insert(vo);
    }

    @PostMapping(value = "updatereply", produces = "text/html;charset=utf-8")
    public void updateReplyAnd(ReplyVO vo) {
        service.update(vo);

    }

    @PostMapping(value = "deletereply", produces = "text/html;charset=utf-8")
